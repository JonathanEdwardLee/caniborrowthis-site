...
describe('P8-01 only S7/S8/S9 source additions', () => {
  it('adds exactly three new frozen sources without unrelated expansion', async () => {
...
describe('P8-02 Springfield GEO pressure washer routes to S7', () => {
  it('selects Springfield tool library before any distant fallback', async () => {
...
describe('P8-03 Springfield GEO 3D printer routes to S8 with on-site wording', () => {
  it('returns maker space resource with explicit not-take-home note', async () => {
    const target = await resolveGeoSearchTarget(
      SPRINGFIELD_COORDS.lat,
      SPRINGFIELD_COORDS.lon,
      '3D printer',
...
describe('P8-04 Mountain Home GEO telescope routes to S9', () => {
  it('selects Baxter special collections before national telescope resource', async () => {
    const target = await resolveGeoSearchTarget(
...
describe('P8-05 Mountain Home GEO ukulele routes to S9', () => {
  it('returns Baxter special collections for ukulele', async () => {
    const target = await resolveGeoSearchTarget(
      MOUNTAIN_HOME_COORDS.lat,
      MOUNTAIN_HOME_COORDS.lon,
      'ukulele',
...
describe('P8-06 ZIP 72653 fishing pole manual search', () => {
  it('returns Baxter special collections on manual ZIP search', async () => {
...
describe('P8-07 Springfield manual ZIP routing and unsupported honesty', () => {
  it('routes ladder at representative Springfield ZIP to S7', async () => {
    const out = search({ objectText: 'ladder', zip: '65807', locationMode: 'ZIP' });
    assert.equal(out.results[0].sourceId, 'S7_SPRINGFIELD_TOOL_LIBRARY');
  });

  it('routes vinyl cutter at representative Springfield ZIP to S8', async () => {
    const out = search({ objectText: 'vinyl cutter', zip: '65802', locationMode: 'ZIP' });
    assert.equal(out.results[0].sourceId, 'S8_SPRINGFIELD_MAKER_SPACE');
  });

  it('keeps unsupported object honest at Springfield ZIP', async () => {
...
describe('P8-08 trust contracts without live availability claims', () => {
  it('S7 links live inventory browse and cites eligibility caveat', async () => {
  ...
  it('S9 tells users to check source for current availability', async () => {
...
describe('P8-09 Pass-006 GEO regressions preserved', () => {
  it('Huntsville sewing machine still avoids fallback-only Fort Payne', async () => {
    const target = await resolveGeoSearchTarget(34.7304, -86.5861, 'sewing machine');
    assert.equal(target.kind, 'zip');
    assert.notEqual(target.zip, '35967');
    assert.equal(target.zip, '16693');
  });

  it('binoculars distant GEO still uses national-only path without ZIP autofill', async () => {
    const target = await resolveGeoSearchTarget(34.7304, -86.5861, 'binoculars');
    assert.equal(target.kind, 'national');
  });

  it('manual ZIP 16693 sewing machine ranking unchanged', async () => {
...
describe('P8-10 pass012 release identity, dependency state, and accessibility', () => {
